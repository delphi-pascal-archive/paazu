...
begin
  ...
  {The chunk name}
  Stream.WriteBuffer(fName, 4);
  {If there is data for the chunk, write it}
  if DataSize > 0 then Stream.WriteBuffer(Data^, DataSize);
...
begin
  ...
  ResizeData(Size);
  if Size > 0 then Stream.ReadBuffer(fData^, Size);
  {Reads CRC}
  Stream.ReadBuffer(CheckCRC, 4);
...
begin
  {Reads the header}
  Stream.WriteBuffer(PNGHeader[0], 8);
  {Write each chunk}
